...
class PerfRoutingTest
  ...
  "The routerGroup" must {
    ...
    "survive killed actor ref routee" in {
      ...
      val router =
        system.actorOf(RoundRobinGroup(List()).props(), "router-test2")
      val props = Props(new GetLicense(endProbe.ref))
      val creator =
        system.actorOf(Props(new WrongDynamicRouteeSizer(2, props, router)),
                       "DynamicRouteeSizer-test2")
      Thread.sleep(100)
    ...
    "survive killed routee" in {
      ...
      val router =
        system.actorOf(RoundRobinGroup(List()).props(), "router-test3")
      val props = Props(new GetLicense(endProbe.ref))
      val creator =
        system.actorOf(Props(new DynamicRouteeSizer(2, props, router)),
                       "DynamicRouteeSizer-test3")
      Thread.sleep(100)
      val msg = PerformanceRoutingMessage(
        ImageProcessing.createPhotoString(new Date(), 60, "123xyz"),
        None,
    ...
    "use manage routees2" in {
      ...
      val router =
        system.actorOf(RoundRobinGroup(List()).props(), "router-test4")
      val props = Props(new GetLicense(endProbe.ref))
      val creator =
        system.actorOf(Props(new DynamicRouteeSizer2(2, props, router)),
                       "DynamicRouteeSizer2-test4")
      Thread.sleep(100)
      val msg = PerformanceRoutingMessage(
        ImageProcessing.createPhotoString(new Date(), 60, "123xyz"),
        None,
    ...
    "Use recreated routees" in {
      ...
      val creator =
        system.actorOf(Props(new GetLicenseCreator2(2, endProbe.ref)),
                       "GetLicenseCreator-test5")
      val paths = List(
        "/user/GetLicenseCreator-test5/GetLicense0",
        "/user/GetLicenseCreator-test5/GetLicense1"
      )
      val router =
        system.actorOf(RoundRobinGroup(paths).props(), "router-test5")
    ...
    "Use recreated routees using broadcast" in {
      ...
      val creator =
        system.actorOf(Props(new GetLicenseCreator2(2, endProbe.ref)),
                       "GetLicenseCreator-test6")
      val paths = List(
        "/user/GetLicenseCreator-test6/GetLicense0",
        "/user/GetLicenseCreator-test6/GetLicense1"
      )
      val router =
        system.actorOf(RoundRobinGroup(paths).props(), "groupRouter-test6")
    ...
    "route between created routers 2" in {
      ...
      val creator =
        system.actorOf(Props(new GetLicenseCreator(2, endProbe.ref)),
                       "GetLicenseCreator2-test7")
      val paths = List(
        "/user/GetLicenseCreator2-test7/GetLicense0",
        "/user/GetLicenseCreator2-test7/GetLicense1"
      )
      val router =
        system.actorOf(RoundRobinGroup(paths).props(), "router-test7")
  ...
  "The Router supervision" must {
    "restart all routees" in {
      val router = system.actorOf(RoundRobinPool(5).props(Props[TestSuper]),
                                  "roundrobinRouter-test1")
      router ! "exception"
      Thread.sleep(1000)
      system.stop(router)
    }
    "restart one routee" in {
      val router =
        system.actorOf(RoundRobinPool(5,
                                      supervisorStrategy =
                                        SupervisorStrategy.defaultStrategy)
                         .props(Props[TestSuper]),
                       "roundrobinRouter-test2")
      router ! "exception"
      Thread.sleep(1000)
      system.stop(router)
    }
  ...
  "The Router" must {
    "routes using roundrobin" in {
      ...
      val endProbe = TestProbe()
      val router =
        system.actorOf(RoundRobinPool(5).props(
                         Props(new GetLicense(endProbe.ref, 250 millis))),
                       "roundrobinRouter")
    ...
    "routes using smallest mailbox" in {
      ...
      val router = system.actorOf(
        SmallestMailboxPool(2).props(Props(new GetLicense(endProbe.ref))),
        "smallestMailboxRouter-test1")
      ...
      processedMessages.size must be(10)
      val grouped = processedMessages.groupBy(_.processedBy)
      val msgProcessedByActor1 = grouped
        .get(Some("250"))
        .getOrElse(Seq())
      val msgProcessedByActor2 = grouped
        .get(Some("500"))
        .getOrElse(Seq())
    ...
    "routes using smallest mailbox with addRoutee" in {
      ...
      val router =
        system.actorOf(SmallestMailboxPool(0).props(
                         Props(new GetLicense(endProbe.ref, 250 millis))),
                       "smallestMailboxRouter-test2")

      val actor1 =
        system.actorOf(Props(new GetLicense(endProbe.ref, 250 millis)), "250")
      val actor2 =
        system.actorOf(Props(new GetLicense(endProbe.ref, 500 millis)), "500")
    ...
    "routes using balancedRouter" in {
      ...
      val router = system.actorOf(
        BalancingPool(2).props(Props(new GetLicense(endProbe.ref))),
        "BalancingPoolRouter"
      )
    ...
    "create routes using BalancingPool and using direct" in {
      ...
      val testSystem = ActorSystem("balance", ConfigFactory.load("balance"))
      val endProbe = TestProbe()(testSystem)
      val router = system.actorOf(
        BalancingPool(2).props(Props(new GetLicense(endProbe.ref))),
        "BalancingPoolRouter2")
