...
//Key: take timeout as implicit property
class BoxOffice(implicit timeout: Timeout) extends Actor {
  ...
  def receive: Receive = {
    ...
    // collect the number of tickets that every ticket seller has left
    case GetEvents =>
      ...
      def convertToEvents(f: Future[Iterable[Option[Event]]]): Future[Events] = f.map(_.flatten).map(l=> Events(l.toVector))
