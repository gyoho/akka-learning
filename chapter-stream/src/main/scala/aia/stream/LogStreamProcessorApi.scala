...
import aia.stream.serialization.{EventMarshalling, LogEntityMarshaller}
...
class LogStreamProcessorApi(
  ...
  val notificationSink = FileIO.toPath(
    notificationsDir.resolve("notifications.json"),
    Set(CREATE, WRITE, APPEND))
  val metricsSink = FileIO.toPath(metricsDir.resolve("metrics.json"),
                                  Set(CREATE, WRITE, APPEND))
  ...
  def getFileSources[T](
      dir: Path): Vector[Source[ByteString, Future[IOResult]]] = {
